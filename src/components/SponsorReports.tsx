...
const SponsorReports: React.FC = () => {
  ...
  const apiUrl = useMemo(() => {
    if (!month) return '/api/sponsors/income-from-sponsors'
    const params = new URLSearchParams({
      month: String(month.getMonth() + 1),
      year: String(month.getFullYear()),
    })
    return `/api/sponsors/income-from-sponsors?${params.toString()}`
  }, [month])
  ...
  const stats = [
    {
      ...
      value: Number(data?.totalSponsoredAmount || 0),
    },
    {
  ...
  return (
    <Gutter>
      ...
      >
        <h2 style={{ margin: 0 }}>Sponsor Payment Summary</h2>
