...
const MemberReports: React.FC = () => {
  ...
  const apiUrl = useMemo(() => {
    if (!month) return '/api/member-payments/income-from-members'
    const params = new URLSearchParams({
      month: String(month.getMonth() + 1),
      year: String(month.getFullYear()),
    })
    return `/api/member-payments/income-from-members?${params.toString()}`
  }, [month])
  ...
  const stats = [
    {
      ...
      value: Number(data?.memberRegistrationFee || 0),
    },
    {
      label: 'Subscription Fee',
      value: Number(data?.memberSubscriptionFee || 0),
    },
    {
      label: 'Total Income',
      value: Number(data?.memberRegistrationFee || 0) + Number(data?.memberSubscriptionFee || 0),
    },
    {
      label: 'Total Due',
